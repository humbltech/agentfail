---
id: "AAGF-2026-009"
title: "LiteLLM PyPI Supply Chain Compromise — TeamPCP Backdoors AI Gateway, Enables Industrial-Scale Credential Theft and Ransomware with Unfixable Encryption Flaw"
status: "published"
...
# Impact
financial_impact: "Not precisely quantified. 25+ organizations hit by Vect ransomware using stolen credentials. Guesty (700 GB claimed), S&P Global (250 GB claimed), and a property-management tech company (4M emails, 700 GB claimed) named as attacker-asserted victims via Halcyon ransomware intelligence. Vect ransomware contains a fatal encryption flaw — files >128 KB are permanently unrecoverable even if ransom is paid. Broader credential rotation costs across all affected organizations are unquantified but substantial."
...
affected_parties:
  count: null                    # Credential exfiltration count unverified; 25+ ransomware victims confirmed
  scale: "widespread"
...
damage_estimate:
  ...
  recovery_cost_usd: null
  averted_damage_usd: 19218750
  averted_range_low: 5000000
  averted_range_high: 50000000
  composite_damage_usd: 19218750
  confidence: "estimated"
  probability_weight: 0.5
  methodology: "25+ ransomware victims confirmed (Halcyon intel). Per-org enterprise breach cost $1.5M (IBM CODB). 25 orgs x $1.5M = $37.5M base. Credential rotation: 50 categories x $350 x 25 orgs = $437,500. Total potential = $37.9M. Probability weight 0.5 = confirmed partial exploitation. Averted = $37.9M x 0.5 = $18.96M rounded to $19.2M."
  methodology_detail:
    per_unit_cost_usd: 1500000
    unit_count: 25
    unit_type: "Ransomware victim organizations (enterprise breach cost per IBM CODB)"
    multiplier: 0.5
    benchmark_source: "IBM 2024 Cost of Data Breach; Halcyon ransomware intelligence; CISA credential rotation guidance"
  estimation_date: "2026-05-06"
  human_override: false
  notes: "Vect ransomware encryption flaw means data >128KB is permanently unrecoverable even with ransom payment. Ransom demands not publicly disclosed. Guesty 700GB and S&P Global 250GB claims are attacker-asserted, not independently verified."

# Presentation
headline_stat: "AI gateway backdoored via 4-hop supply chain: 50+ secret categories harvested in 3 hours, 25+ ransomware victims with permanently unrecoverable data"
...
researcher_notes: "Exceptionally well-sourced incident with corroboration from 5+ Tier-1 security research firms (Unit 42, Snyk, Datadog, Kaspersky, Microsoft). Key discrepancy: LiteLLM claims ~40 minutes exposure while Snyk and FutureSearch timelines indicate ~3 hours. The 3-hour figure is used throughout as it aligns with independent timestamps. The 500K credential exfiltration claim originates from vx-underground sources cited by Unit 42 and could not be independently verified by BleepingComputer (many may be duplicates). This figure is excluded from the headline_stat per council direction and qualified as unverified wherever referenced. The Vect ransomware encryption flaw is independently confirmed by Check Point Research, The Register, and CSA Labs. Discovery via MCP plugin transitive dependency in Cursor is a significant detail — the AI development toolchain itself surfaced the compromise. Named ransomware victims (Guesty, S&P Global) are attacker-asserted via Halcyon ransomware intelligence, not independently confirmed breaches."
...
# LiteLLM PyPI Supply Chain Compromise — TeamPCP Backdoors AI Gateway, Enables Industrial-Scale Credential Theft and Ransomware with Unfixable Encryption Flaw
...
| 2026-04-15 | Vect publishes first victims: Guesty (700 GB claimed), S&P Global (250 GB claimed), property-management company (4M emails, 700 GB claimed) |
...
Check Point Research discovered a fatal flaw in the Vect ransomware deployed against victims compromised through stolen credentials. The ransomware uses AES-256 encryption but discards 3 of 4 decryption nonces for files larger than 128 KB. This means data is **unrecoverable with known techniques** even if the ransom is paid. Check Point characterized it as "ransomware by design, wiper by accident."
...
**Why 5 / Root cause:** Unpinned CI/CD dependencies combined with overly-scoped publishing tokens created a chain of trust that could be broken at any hop, with each break cascading to all downstream consumers. The impact was amplified by LiteLLM's role as an AI gateway that concentrates access to all LLM API keys, cloud credentials, and infrastructure secrets in a single dependency — making this common supply chain failure mode catastrophically worse when it affects AI infrastructure.

**Root cause summary:** Two specific, actionable root causes — unpinned security scanner versions in CI/CD and insufficiently isolated publishing credentials — enabled a multi-hop supply chain compromise. The AI-gateway architecture of the target amplified the blast radius because AI gateways by design aggregate high-value credentials for 100+ LLM providers in a single dependency.
...
**Who was affected:**
- Organizations that installed LiteLLM v1.82.7 or v1.82.8 from PyPI during the ~3-hour exposure window (subset of 3.4M daily downloaders; actual execution count unknown)
- 25+ organizations targeted by Vect ransomware using credentials stolen in this campaign
- Named attacker-claimed victims (unverified): Guesty (700 GB claimed stolen), S&P Global (250 GB claimed), property-management technology company (4M emails, 700 GB claimed)
...
- Users affected: Credential exfiltration scale unverified (a claim of ~500K exfiltrations originates from vx-underground via Unit 42 but could not be independently verified and likely includes significant duplication); 25+ ransomware victims confirmed
...
| Palo Alto Unit 42 | https://unit42.paloaltonetworks.com/teampcp-supply-chain-attacks/ | 2026-03-27 | High — TeamPCP attribution, CanisterWorm analysis |
