...
agent_type: ["Task Automation Agent"]   # TAXONOMY NOTE: OmniGPT is a multi-model chat relay, not an autonomous agent. "AI Platform" is the accurate descriptor but is not a valid AgentFail taxonomy value. Closest valid value is "Task Automation Agent" (the platform automates task routing across multiple LLM backends). Classified here for database consistency; see researcher_notes for full qualification rationale. Included in AgentFail because AI aggregator platforms are infrastructure for agentic deployments and the data governance lessons apply directly to operators running agents via similar relay architectures.
...
| Point | Action |
|-------|--------|
| `agent_type: ["Tool-Using Agent"]` — incorrect | Fixed: changed to `["Task Automation Agent"]` (closest valid taxonomy value) with inline rationale for non-agentic inclusion and taxonomy deviation note in researcher_notes |
