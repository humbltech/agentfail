...
potential_damage: "High — developer workstation RCE via prompt-injected git hook. A successfully exploited instance would give attacker full OS-user-level access on the developer's machine: SSH keys, cloud credentials, source code, browser credential stores, CI/CD pipeline injection via malicious commits. The hook persists indefinitely after agent session teardown, firing on every subsequent matching git operation (checkout, merge, commit, rebase) until manually removed. 5M+ Cursor users were in the at-risk population during the 4-day unpatched window; subset with active agent use against external repositories is the actual exposed population."
intervention: "Coordinated disclosure by Novee Security Research Team, Daniel Teixeira (Nvidia AI Red Team), and Philip Tsukerman allowed Anysphere to patch in 4 days (Cursor 2.5, released February 17, 2026) before broad public awareness. No public PoC was published. Patch added explicit OS-level sandbox deny rules blocking agent writes to .git/hooks/ and .git/config on all platforms (Seatbelt/macOS, Landlock+seccomp/Linux, WSL2/Windows). No confirmed exploitation during the 4-day window."
...
damage_estimate:
  ...
  notes: "Probability weight set at 0.001 (near-miss conditions: 4-day patch window, no public PoC, requires social engineering to open malicious repo, coordinated disclosure before broad awareness). NIST CVSS 9.9 CRITICAL reflects theoretical worst case; CNA CVSS 8.0 HIGH reflects actual attack bar. The 5M unit count uses the full user base; the realistic at-risk population (active agent users who open external repositories) is likely 5–10% of that, suggesting a more defensible averted_damage_usd of $2.5M–$5M. The $50M figure represents the ceiling with full-user-base assumption. Averted damage figure should be read as order-of-magnitude orientation, not precise estimate."

# Presentation
headline_stat: "AGENTS.md — the probable primary injection vector: one malicious repo primes Cursor's agent to write a backdoor git hook that executes on the next git operation, surviving agent session teardown"
...
# Relationships
related_incidents:
  - "AAGF-2026-043"   # TrustFall (Claude Code CVE-2026-33068) — same pattern: repo config file weaponizes agent autonomy → RCE (permission-field bypass variant)
  - "AAGF-2026-062"   # TrustFall Adversa AI replication — independent confirmation of config-file trust abuse
  - "AAGF-2026-066"   # Gemini CLI CVSS 10.0 — .gemini/ config dirs execute before sandbox initializes; same root cause class (pre-sandbox execution variant)
...
Note: The GHSA advisory (primary source, high confidence) describes the vulnerability as prompt injection via "attacker-controlled repository content" — it does not name AGENTS.md specifically. AGENTS.md is the highest-confidence probable primary vector given its purpose in the ecosystem, corroborated by the IOC scanner patterns in agentic-ioc-scanner §9, but the vulnerability affects any file the agent reads as instructional context from an attacker-controlled repository.
...
Open question: the Cursor 2.5 blog post uses language suggesting "new sandbox implementation" rather than "sandbox with a missing deny rule" — if the sandbox was entirely absent in versions prior to 2.5, the vulnerability window extends back to whenever the AI agent feature was introduced, not just the post-2.4 "git writes allowed" change. This distinction is material for assessing historical exposure and is unresolved from public sources.
...
**Proximate cause:** Cursor's AI agent, following prompt-injected instructions from a malicious repository's content, wrote a hook script to `.git/hooks/` — a path not restricted by the pre-2.5 sandbox — and git subsequently executed the hook outside the sandbox.
...
**Why 4: Why did the agent treat repository content instructions as authoritative enough to execute repository configuration changes?**
...
- Averted damage estimate: ~$50M (ceiling with full user base; ~$2.5M–$5M with realistic at-risk sub-population; see `damage_estimate`)
...
| Incident | Connection |
|----------|------------|
| AAGF-2026-043 (TrustFall — Claude Code CVE-2026-33068) | Same pattern: repo-local config file (`.claude/settings.json` vs. AGENTS.md) weaponizes agent trust to achieve RCE. Permission-field bypass variant — TrustFall is one-step (immediate permission bypass); this is two-step (hook persists for future execution). Both are "agent as victim" attacks. |
...
| AAGF-2026-066 (Gemini CLI CVSS 10.0) | Pre-sandbox execution variant — closest structural analog: `.gemini/` config directories execute before sandbox initializes — same root cause (agent config files trigger code execution before or without sandbox controls). Different trigger mechanism (pre-sandbox initialization vs. git hook delayed execution). |
...
CVE-2026-26268 is a valid, well-sourced entry that earns its place as the third confirmed instance of the `coding-agent-config-trust` class. The core analysis is sound, the 5 Whys are well-constructed, and the solution analysis is thorough.

**On severity "high":** Confirmed correct. The `actual_vs_potential: near-miss` field correctly signals the absence of confirmed victims; `severity: "high"` correctly signals the consequence class if exploited. These fields are not in conflict. The judgment call at the medium/high boundary is acknowledged explicitly.

**On AGENTS.md framing:** Corrected in this publication. Headline_stat and Technical Analysis opening paragraph now use "probable primary vector" qualifier per council Issue 1. The vulnerability affects any attacker-controlled repository content the agent reads as instructional context.

**On damage estimate:** Notes updated to reflect that $50M is a ceiling assuming full user base; realistic at-risk sub-population ($2.5M–$5M) is more defensible. Unit count limitation acknowledged in notes.

**On pattern group sub-variants:** Related Incidents table updated with explicit sub-variant labels (permission-field bypass, pre-sandbox execution, agent-planted persistent artifact) to distinguish the three mechanistically distinct members of the coding-agent-config-trust class.

**On separate entry justification:** Confirmed warranted. The temporal-decoupling characteristic (asynchronous hook execution, attacker-controllable blast radius timing, forensic opacity) distinguishes this entry from TrustFall and Gemini CLI in ways that matter to operators.

**On pattern group:** Confirmed warranted at three vendor-diverse confirmed instances. Group created with this publication.
...
- Whether the Cursor agent sandbox was entirely absent (not merely incomplete) in versions prior to 2.5 — if so, the vulnerability window extends to all AI agent versions, not just post-2.4 "git writes allowed" builds.
- Whether Assaf Levkovich is part of the Novee Security Research Team — treat his role as unconfirmed until reconciled.
...
4. **The coding agent RCE pattern is now a class vulnerability.** CVE-2026-26268 is the third confirmed instance of AI coding tools creating RCE pathways through local config file trust (TrustFall/Claude Code, Gemini CLI, Cursor). Vendors of agentic coding tools should treat this as a shared threat model requiring cross-ecosystem coordination, not isolated implementation bugs requiring individual patches.
